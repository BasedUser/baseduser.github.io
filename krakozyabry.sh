...
install_layout() {
  ...
  ru_MB          Russian\ \(Mojibake\)' "$EVDEV_LST"
    ...
    echo -e "${YELLOW}Теперь вы можете добавить расскладку Russian (Mojibake), используя удобный вам редактор настроек."
    echo -e "Если раскладка не применяется после добавления, перезапустите графическую сессию или выполните logout/login.${CLEAR}"
}
